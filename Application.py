...
class Application:
    ...
    def video_loop(self):
        ...
        if ok:
            ...
            predicts_list = list(predicts)

            if(len(predicts_list) > 1):
               self.bt1.config(text = predicts_list[0], font = ("Courier", 20))
            else:
                self.bt1.config(text = "")

            if(len(predicts_list) > 2):
               self.bt2.config(text = predicts_list[1], font = ("Courier", 20))
            else:
                self.bt2.config(text = "")

            if(len(predicts_list) > 3):
               self.bt3.config(text = predicts_list[2], font = ("Courier", 20))
            else:
    ...
    def action3(self):
        ...
        if len(predicts) > 2:
            self.word = ""
            self.str += " "
            self.str += list(predicts)[2]
    def action4(self):
        predicts = self.spell.candidates(self.word)
        if len(predicts) > 3:
            self.word = ""
            self.str += " "
            self.str += list(predicts)[3]
